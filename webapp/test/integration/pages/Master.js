			...
			onTheMasterPage : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("Kunnr") === oObject.Kunnr;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].Kunnr;
										} else {
											sCurrentId = aItemsNotInTheList[0].Kunnr;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("ZC4C_KLANTSet", {
											Kunnr : sCurrentId
										});
				...
				assertions : {
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all ZC4C_KLANTSet to be able to check for the total amount
